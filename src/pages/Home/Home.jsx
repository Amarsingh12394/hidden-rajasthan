...
const Home = () => {
  ...
  return (
    <div>
      ...
      <section className="container mx-auto px-4 py-8"><h2 className="text-2xl font-bold mb-4">Featured Categories</h2><div className="grid grid-cols-2 md:grid-cols-4 gap-4">{categories.slice(0,4).map(cat => <CategoryCard key={cat.slug} category={cat} />)}</div></section>

      <section className="container mx-auto px-4 py-8"><h2 className="text-2xl font-bold mb-4">Best Selling Products</h2><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">{bestSellers.map(p => <ProductCard key={p.id} product={p} />)}</div></section>

      <section className="bg-amber-50 py-12 text-center"><h2 className="text-2xl font-bold">Why Hidden Rajasthan?</h2><p className="max-w-xl mx-auto mt-2">Direct from artisans • Ethical sourcing • Global shipping</p></section>
